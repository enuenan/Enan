    ...
    <div class="mx-auto max-w-7xl">
        ...
        <div class="grid gap-8 lg:grid-cols-2">
            @foreach ($personal_works as $personal_work)
                <article
                    class="group overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.06] shadow-2xl shadow-black/20 backdrop-blur-xl transition duration-300 hover:-translate-y-2 hover:border-emerald-300/30">
                    <a href="{{ $personal_work['link'] }}" target="_blank" rel="noopener noreferrer"
                        class="block overflow-hidden">
                        <img src="{{ asset($personal_work['image']) }}"
                            class="h-80 w-full object-cover transition duration-700 group-hover:scale-105"
                            alt="{{ $personal_work['title'] }}">
                    </a>
                    <div class="p-8">
                        <h3 class="text-2xl font-black text-white">
                            <a href="{{ $personal_work['link'] }}" target="_blank" rel="noopener noreferrer"
                                class="transition hover:text-emerald-300">{{ $personal_work['title'] }}</a>
                        </h3>
                            ...
                            @if (!empty($personal_work['social']))
                                @foreach ($personal_work['social'] as $social)
                                    <a href="{{ $social['link'] }}" target="_blank" rel="noopener noreferrer"
                                        title="{{ $social['title'] }}"
                                        class="inline-flex h-12 w-12 items-center justify-center rounded-full border border-white/10 bg-white p-2 transition hover:-translate-y-1 hover:border-emerald-300">
